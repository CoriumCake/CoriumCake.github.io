...
export default class DinoGame extends GameRunner {
  ...
  updateScore() {
    ...
    if (this.frameCount % state.settings.scoreIncreaseRate === 0) {
      ...
      if (state.level !== oldLevel) {
        playSound('level-up')
        // this.increaseDifficulty() //disable increase difficulty
  ...
  drawScore() {
    ...
    if (shouldDraw) {
      ...
      this.paintText((drawValue + '/999').padStart(5, '0'), this.width, 0, {
        font: 'PressStart2P',
        size: `${fontSize}px`,
        align: 'right',
        baseline: 'top',
